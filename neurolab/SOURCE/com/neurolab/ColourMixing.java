...
public class ColourMixing extends NeurolabExhibit{
  ...
  private JPanel             squares, innersquare, fillsquare, fillsquare2,
      fillsquare3, sliderbarpanel, colourdisplaypanel;
  ...
  private GridBagConstraints mainGbConstraints, squaresGbConstraints,
      cdpGbConstraints;
  ...
  public boolean             equiluminantTriangle = true;
  
  public double              length       = 200;
  public double              angle        = Math.PI / 3;                    // 60
                                                                            // deg
  public int                 height       = (int)(length * Math.sin(angle));
  public int                 radius       = (int)(0.2f * height);
  public Point               circ_cent    = new Point((int)(length - (Math
                                              .cos(angle / 2)
                                              * height * 0.4f)),
                                              (int)(0.8f * height));
  ...
  public void createComponents(){
    ...
    /*
     * now added inside the sliderbarpanel //add the foreground sliders
     * mainGbConstraints.fill = GridBagConstraints.VERTICAL; addMainComponent(
     * fore, 0, 1, 1, 2, 10, 0 );
     * 
     * //add the background sliders mainGbConstraints.fill =
     * GridBagConstraints.VERTICAL; addMainComponent( back, 2, 1, 1, 2, 10, 0 );
     */
    ...
    /*
     * now added inside the colourdisplay panel //add the section
     * mainGbConstraints.fill = GridBagConstraints.BOTH; addMainComponent(
     * section, 1, 2, 1, 1, 5, 10 );
     * 
     * //add the complement box mainGbConstraints.fill =
     * GridBagConstraints.BOTH; addMainComponent( complement, 2, 2, 1, 1, 5, 10 );
     * 
     * //add the coloured squares display squares = new JPanel();
     * mainGbConstraints.fill = GridBagConstraints.BOTH; addMainComponent(
     * squares, 0, 2, 1, 1, 5, 20 );
     */
    ...
    innersquare.setBackground(new Color(forecolours[0], forecolours[1],
        forecolours[2]));
    squares.setBackground(new Color(backcolours[0], backcolours[1],
        backcolours[2]));
  ...
  public void paint(Graphics g) // gets called on by repaint or by window
                                // covering etc etc
  {
    fore.repaint();
    back.repaint();
  ...
  private class InputHandler implements ItemListener, ChangeListener,
    ...
    public void itemStateChanged(ItemEvent e){
      ...
      if(e.getStateChange() == ItemEvent.DESELECTED
          && source == buttonpanel.randomButton){
        timer.stop();
      }
      
      if(e.getStateChange() == ItemEvent.SELECTED
          && source == buttonpanel.constantButton){
        equiluminant = true;
        calculate();
        repaint();
      }
    ...
    public void mouseClicked(MouseEvent e){
      if(e.getSource() == triangle){
        ...
        if((e.getY() > triangle.northcorner.y) && (e.getY() < triangle.westcorner.y)
            && (e.getX() < triangle.eastcorner.x - (triangle.westcorner.y-e.getY())*0.58)
            && (e.getY() > triangle.westcorner.x + (e.getY()-triangle.northcorner.y)*0.58) 
           )
            //(120 - (e.getY() - 76) * 0.58f))
            //&& (e.getX() < (120 + (e.getY() - 76) * 0.58f))) // if within
                                                              // triangle
        {
          ...
          Point red_corner = triangle.westcorner;//new Point(20, 249);
          Point green_corner = triangle.eastcorner;//new Point(219, 249);
          Point blue_corner = triangle.northcorner;//new Point(119, 76);
          
          int red_dist = (int)ptLineDist(green_corner, blue_corner, e
              .getPoint());
          int green_dist = (int)ptLineDist(red_corner, blue_corner, e
              .getPoint());
          int blue_dist = (int)ptLineDist(red_corner, green_corner, e
              .getPoint());
          ...
          double scaling = 255.0f / (Math.max(red_dist, (Math.max(green_dist,
              blue_dist))));
          ...
          if(e.isMetaDown() || e.isControlDown() || e.isAltDown()) back.setValues(red_dist, green_dist, blue_dist);
...

  {
    ...
    for(int y = 0; y < height; y++){
      ...
      for(int x = 0; x < length; x++){
        if(x >= (length / 2 - xinc) && (x <= (length / 2 + xinc))) // inside
                                                                    // triangle
        {
          // int z = height - y;
          
          redcomp = (int)(ratio * ptLineDist((length / 2), 0, length, height,
              x, y));
          greencomp = (int)(ratio * ptLineDist((length / 2), 0, 0, height, x, y));
          bluecomp = (int)(ratio * ptLineDist(0, height, length, height, x, y));
          
          if(equiluminantTriangle) scaling = (255.0f / Math.max(redcomp, (Math
              .max(greencomp, bluecomp))));
          else scaling = 1/170f*(forecolours[0]+forecolours[1]+forecolours[2])/768f; // /256.../768/40
          
          redcomp = (int)(redcomp * scaling);
          greencomp = (int)(greencomp * scaling);
          bluecomp = (int)(bluecomp * scaling);
          
          if(distance(circ_cent, x, y) > radius && x > circ_cent.x
              && y > (circ_cent.y - 10)){
            redcomp = 191;
            greencomp = 191;
            bluecomp = 191;
    ...
    colourtriangle = createImage(new MemoryImageSource((int)length,
        (int)height, pix, 0, (int)length));
  ...
  class Triangle extends JPanel{
    ...
    Point westcorner, eastcorner, northcorner;
    public void paintComponent(Graphics g){
       ...
       */
      westcorner = new Point(gap, 4 * gap + colourtriangle.getHeight(this));
      eastcorner = new Point(gap + colourtriangle.getWidth(this), 4 * gap
          + colourtriangle.getHeight(this));
      northcorner = new Point((gap + (int)length / 2), 4 * gap);
      
      g.drawImage(colourtriangle, gap, 4 * gap, this);
      /*
       * GeneralPath triangle_area = new GeneralPath();
       * triangle_area.moveTo((gap+length/2),4*gap); triangle_area.lineTo(gap +
       * colourtriangle.getWidth(this),4*gap + =
       * colourtriangle.getHeight(this)); triangle_area.lineTo(gap,4*gap +
       * colourtriangle.getHeight(this));
       * //triangle_area.lineTo((gap+length/2),gap); triangle_area.closePath();
       */
      g2.setColor(Color.black);
      g2.drawLine(northcorner.x, northcorner.y, eastcorner.x, eastcorner.y);
      g2.drawLine(eastcorner.x, eastcorner.y, westcorner.x, westcorner.y);
      g2.drawLine(westcorner.x, westcorner.y, northcorner.x, northcorner.y);
      ...
      Point centre = new Point((westcorner.x + (colourtriangle.getWidth(this) / 2)),
          (westcorner.y - (colourtriangle.getHeight(this) / 2)));
      
      Point foreg = new Point(
          (int)(centre.x + ((1 - f_r) * (colourtriangle.getWidth(this) / 2)) - ((1 - f_g) * (colourtriangle
              .getWidth(this) / 2))),
          (int)(centre.y - ((f_b) * (colourtriangle.getHeight(this) / 2)) + ((1 - f_b) * (colourtriangle
              .getHeight(this) / 2))));
      ...
      Point backg = new Point(
          (int)(centre.x + ((1 - b_r) * (colourtriangle.getWidth(this) / 2)) - ((1 - b_g) * (colourtriangle
              .getWidth(this) / 2))),
          (int)(centre.y - ((b_b) * (colourtriangle.getHeight(this) / 2)) + ((1 - b_b) * (colourtriangle
              .getHeight(this) / 2))));
      ...
      if((foreg.x > 0) && (backg.x > 0)) // draw line fore(x,y) to back(x,y)
      {
        ...
        if(right.x > new_circ_cent.x && distance(new_circ_cent, right) > radius) do{
          int tempx = right.x - 1;
          int tempy = right.y - (int)grad;
          right.setLocation(tempx, tempy);
        }while(right.x > new_circ_cent.x
            && distance(new_circ_cent, right) > radius);
        
        if((left.y >= 75) && (left.y <= 270)
            && (left.x >= (120 - (left.y - 75) * 0.58f))
            && (left.x <= (120 + (left.y - 75) * 0.58f)) && (right.y >= 75)
            && (right.y <= 270) && (right.x >= (120 - (right.y - 75) * 0.58f))
            && (right.x <= (120 + (right.y - 75) * 0.58f))) g2.drawLine(
      ...
      g2.drawString("B", (int)(northcorner.x - 10), (int)(northcorner.y - 5));
      // G
      g2.setColor(Color.green);
      g2.drawString("G", (int)(eastcorner.x - 10), (int)(eastcorner.y + 30));
      // R
      g2.setColor(Color.red);
      g2.drawString("R", (int)(westcorner.x - 10), (int)(eastcorner.y + 30));
    } // ends paint
  ...
  class Section extends JPanel{
    ...
    public void paintComponent(Graphics g){
      ...
      for(int x = 0; x < 100; x++){
        ...
        if(equiluminant) scaling = (255.0f / Math.max(newred, (Math
            .max(newgreen, newblue))));
        else scaling = 1;
...
//      ((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
      ...
      g2.setColor(new Color((255 - forecolours[0]), (255 - forecolours[1]),
          (255 - forecolours[2])));
      g2.fillRoundRect(30, 0, 30, 30, 2, 2);
  ...
  class buttonPanel extends JPanel{
    ...
    public buttonPanel(){
      ...
      // comment this line to remove the equiluminance button ******
     //  add( constantButton ); // add equiluminance checkbox
      ...
      /*
       * spacer3 = new Spacer(5,5); spacer3.setBackground(Color.red);
       * buttonsGbConstraints.weightx = 2; buttonsGbConstraints.weighty = 2;
       * buttonsGbConstraints.gridx = 0; buttonsGbConstraints.gridy = 6;
       * buttonsLayout.setConstraints( spacer3, buttonsGbConstraints ); //add(
       * spacer3 ); // add blackout button
       * 
       * setLayout(new GridLayout(4,1,5,5)); add(blackout,BorderLayout.EAST );
       * add(randomButton,BorderLayout.EAST ); add(whiteout,BorderLayout.EAST );
  ...
  class SliderControls extends JPanel{
    ...
    public SliderControls(ColourMixing app, int redstart, int greenstart,
      ...
      if(foreground) title1 = BorderFactory.createTitledBorder(BorderFactory
          .createEtchedBorder(), "Foreground");
      else title1 = BorderFactory.createTitledBorder(BorderFactory
          .createEtchedBorder(), "Background");
